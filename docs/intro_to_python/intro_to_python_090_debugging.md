...
In practice, scientists tend to do the following:
...
3. *Compare to an oracle.*
    A [test oracle](python_reference.md#test-oracle) is something whose results are trusted,
...
4. *Check conservation laws.*
    ...
    Similarly, if we are analyzing patient data,
...
i.e., that we're actually exercising the problem that we think we are.
...
i.e., we want to localize the failure to the smallest possible region of code:
...
Every time we make a change, however small,
...
take a deep breath, and start working more systematically.

Records are particularly useful when the time comes to ask for help.
People are more likely to listen to us when we can explain clearly what we did,
...
we want it to work, so we're likely to keep telling ourselves why it should,
...
In either case, taking note of why the error occurred and checking for it next time
